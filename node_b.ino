...
/*
 * Peer-to-Peer Smart IoT Water Grid - Node B
 * 
 * Hardware (Identical to Node A):
 * - ESP32 DevKit
 * - Potentiometer 1 (GPIO 34): Initial Tank Level / Manual Override
...
// Node Identity - SWAPPED FOR NODE B
const char* NODE_ID = "B";
const char* PEER_ID = "A";

// Pins
const int PIN_LEVEL_INIT = 34;   
const int PIN_CURRENT    = 35;   
const int PIN_PUMP_MAIN  = 26;   
const int PIN_PUMP_BACKUP= 27;   
...
const float LOW_THRESHOLD = 40.0; // Request water if below this
...
const long REQUEST_INTERVAL = 5000;
...
void setup() {
  ...
  pinMode(PIN_LEVEL_INIT, INPUT); 

  // Init LCD
  lcd.init();
  lcd.backlight();
  lcd.setCursor(0, 0);
  lcd.print("Node B Init...");

  // Initial Read
  lastPotRaw = analogRead(PIN_LEVEL_INIT);
  tankLevel = map(lastPotRaw, 0, 4095, 0, 100);
...
void updateLCD() {
  ...
  lcd.setCursor(0, 0);
  lcd.printf("ID:%s Lvl:%.0f%%  ", NODE_ID, tankLevel); 
...
void reconnect() {
  while (!client.connected()) {
    ...
    String clientId = "WG_2026_NodeB_";
